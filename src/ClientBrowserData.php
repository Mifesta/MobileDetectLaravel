 ...
 * @method static boolean setCfHeaders(array | null $cfHeaders)
 * @method static array getCfHeaders()
 * @method static string|null getUserAgent()
 * @method static string|null setDetectionType($type)
 ...
 * @method static string|float version(string $propertyName, string | null $type)
...
abstract class ClientBrowserData
{
	...
	{
		return preg_match('#(bot|spider|yeti|ichiro)#is', self::getMobileDetect()->getUserAgent())
			|| self::getMobileDetect()->is('MobileBot')
			|| self::getMobileDetect()->is('Bot');
	}
	...
	public static function isBot()
	{
		...
		return $classMobileDetect->is('Bot')
			|| preg_match('#(yeti|ichiro)#is', $user_agent)
			|| (preg_match('#(bot|spider)#is', $user_agent) && !$classMobileDetect->is('MobileBot'));
	}
